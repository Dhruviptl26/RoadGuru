...
public class GoogleMapsActivity extends FragmentActivity implements
        ...
        navView.setOnNavigationItemSelectedListener(new BottomNavigationView.OnNavigationItemSelectedListener() {
            ...
            public boolean onNavigationItemSelected(@NonNull MenuItem item) {
                if (item.getItemId() == R.id.navigation_parking) {
                    ...
                    showNearbyPoliceStations();
                    return true;
                } else if (item.getItemId() == R.id.navigation_parking2) {
                    showNearbyHospitals();
                    return true;
                } else {
                    return false;
    ...
    @Override
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        if (requestCode == Request_User_Location_Client) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                if (ContextCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
                    buildGoogleApiClient();
                    mMap.setMyLocationEnabled(true);
        ...
        if (isLocationAvailable && lastlocation != null) {
            ...
            new Thread(() -> {
                try {
                    ...
                    parseJsonResponse(jsonResponse.toString(), "parking", BitmapDescriptorFactory.HUE_BLUE);
                } catch (IOException e) {
                    runOnUiThread(() -> Toast.makeText(GoogleMapsActivity.this, "Error fetching data", Toast.LENGTH_SHORT).show());
                }
            }).start();
        ...
        if (lastlocation != null) {
            ...
            new Thread(() -> {
                try {
                    ...
                    parseJsonResponse(jsonResponse.toString(), "police", BitmapDescriptorFactory.HUE_RED);
                } catch (IOException e) {
                    runOnUiThread(() -> Toast.makeText(GoogleMapsActivity.this, "Error fetching data", Toast.LENGTH_SHORT).show());
                }
            }).start();
        ...
        if (lastlocation != null) {
            ...
            new Thread(() -> {
                try {
                    ...
                    parseJsonResponse(jsonResponse.toString(), "hospital", BitmapDescriptorFactory.HUE_ORANGE);
                } catch (IOException e) {
                    runOnUiThread(() -> Toast.makeText(GoogleMapsActivity.this, "Error fetching data", Toast.LENGTH_SHORT).show());
                }
            }).start();
    ...
    private void parseJsonResponse(String jsonResponse, String placeType, float markerColor) {
        runOnUiThread(() -> {
            try {
                JSONObject jsonObject = new JSONObject(jsonResponse);
                ...
                for (int i = 0; i < resultsArray.length(); i++) {
                    ...
                    mMap.addMarker(new MarkerOptions()
                            ...
                            .title(placeName + " (" + placeType + ")")
                            .icon(BitmapDescriptorFactory.defaultMarker(markerColor)));
                }
            } catch (JSONException e) {
                Toast.makeText(GoogleMapsActivity.this, "Error parsing JSON", Toast.LENGTH_SHORT).show();
            }
        });
    }
}
